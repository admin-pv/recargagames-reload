 ...
 */
export async function sendWelcome(cfg, { voucherId, email, locale, siteHost, label }) {
  ...
  const { subject, html, text } = welcomeEmail({ locale, siteHost });
 ...
 */
export async function sendPinEmail(cfg, { attemptId, email, locale, siteHost, pin, serial, productLabel, label }) {
  if (!email || !pin || !mailerEnabled()) return false;

  const { subject, html, text } = pinEmail({ locale, siteHost, pin, serial, productLabel });
  const result = await sendEmail({ to: email, subject, html, text });
