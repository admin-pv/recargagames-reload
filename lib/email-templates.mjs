...
const SITE = "https://reload.recargagames.com";

/** Idiomas com tradução completa. Fora daqui, cai no default. */
export const SUPPORTED_LOCALES = ["pt-BR", "es-MX"];
export const DEFAULT_LOCALE = "pt-BR";

/**
 * Normaliza o locale pedido. Desconhecido → default, sem estourar: um
 * email em português é infinitamente melhor que um resgate sem email
 * porque o locale veio errado.
 */
...
const COPY = {
  "pt-BR": {
    ...
    footerHelp: "Precisa de ajuda? Este email não recebe respostas — fale com o suporte.",
    footerSite: "reload.recargagames.com",
  ...
  "es-MX": {
    ...
    footerHelp: "¿Necesitas ayuda? Este correo no recibe respuestas — contacta a soporte.",
    footerSite: "reload.recargagames.com",
    ...
    <tr>
      <td style="padding:8px 32px 32px 32px;border-top:1px solid ${BRAND.border};">
        ...
        <p style="margin:0 0 4px 0;font:400 12px/1.6 Helvetica,Arial,sans-serif;color:${BRAND.muted};">
          <a href="${SITE}" style="color:${BRAND.cyan};text-decoration:none;">${esc(copy.footerSite)}</a>
        </p>
...
function shell(locale, copy, inner) {
  return `<!doctype html>
 ...
 */
export function welcomeEmail({ locale } = {}) {
  ...
  return { subject: copy.welcomeSubject, html: shell(lang, copy, inner), text };
}
 ...
 */
export function pinEmail({ locale, pin, serial, productLabel } = {}) {
  const lang = resolveLocale(locale);
  const copy = COPY[lang];
  ...
  return { subject: copy.pinSubject, html: shell(lang, copy, inner), text };
}
